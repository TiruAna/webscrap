load("C:/Users/ana.tiru/Desktop/emag/emaglinks.rda")
...
emag_get_products <- function(emag.links = NULL){
  ...
  emag.links$url_produs <- as.character(emag.links$url_produs)
  dfg <- emag.links %>% group_by(categorie) %>% summarise(n=n())
  counter <- 0
  
  for(i in 1:nrow(emag.links)){
    counter <- counter + 1
    ...
    skip_next <- 0
    err <- tryCatch(expr = {
            y <- httr::GET(emag.links$url_produs[i])
    ...
    categor <- emag.links$categorie[i]
    s <- subset(dfg, dfg$categorie == categor)
    
    if (counter == s$n[1]) {
      write.csv(produse, paste0(categorie, "_produse",".csv"), row.names = FALSE)
      write.csv(logger, paste0(categorie, "_logger", ".csv"), row.names = FALSE)
      counter <- 0
    }
  }
...
df = bind_rows(emag.links, .id = "column_label")

dfg <- df %>% group_by(categorie) %>% summarise(n=n())
